...
class DobraCutanea {
    ...
    public function buscarComposicaoCorporalPorId($id) {
        $sql = $this->db->prepare("SELECT * FROM tb_dobras_cutanea WHERE id = :id");
        $sql->bindParam(':id', $id);
        $sql->execute();
    ...
    public function buscarComposicaoCorporalPorUsuarioId($usuario_id) {
        $sql = $this->db->prepare("SELECT * FROM tb_dobras_cutanea WHERE usuario_id = :usuario_id ORDER BY data_avaliacao DESC");
